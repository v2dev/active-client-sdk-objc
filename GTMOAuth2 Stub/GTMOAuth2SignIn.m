...
- (void)handleCallbackReached {
  ...
  if ([code length] > 0) {
      ...
      /**
       * This is in place of the code below.  Should just finish out the processing
       */
      [self finishSignInWithError:nil];
      /**
       * Modified by Jonathan Samples - below is the normal code
       * I've commented it out so that we can use the authorization_code to
       * get the token from the server and keep it there.
       */ 
