 ...
 withConnectionTimeout:(NSTimeInterval)connectionTimeout
{
    if (!_isConnected && !_isConnecting) {
        ...
        NSString *port = _port ? [NSString stringWithFormat:@":%@", @(_port)] : @"";
        ...
        if (_handshake) {
            ...
            // connection failed
            [self connection:_handshake didFailWithError:[NSError errorWithDomain:@"handshake error" code:500 userInfo:nil]];
        }
    }
}
...
- (void) connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response 
{
    ...
    if ([response respondsToSelector:@selector(statusCode)]) {
        ...
        DEBUGLOG(@"didReceiveResponse() %@", @(statusCode));
